...
FILE_NAME: linear_regression
AUTHOR: welt
E_MAIL: tjlwelt@foxmail.com
DATE: 2022/07/16
Description: 对输入的三张栅格图像进行逐像元的线性回归，并将得到的a，b，p值写入新的栅格图像中。
"""
import numpy as np
import sklearn
...
def regression(x, y):
    """
    :param x: n行2列的numpy数组，n为年份数，第一列为气温，第二列为降水
    :param y: 目标：n个元素的numpy数组，为NDVI
    :return: a, b, c
    """
    ...
    for row in tqdm(range(rows)):
        for col in range(colmns):
            ...
            if (np.array(x) > -9999).all() and (np.array(y) > -9999).all():
                ...
                out_a[row, col], out_b[row, col] = a, b
